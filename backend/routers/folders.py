...
def _find_folder_by_name(db: Session, name: str, *, exclude_folder_id: str | None = None) -> Folder | None:
    query = db.query(Folder).filter(func.lower(Folder.name) == name.lower())
...
def get_folders(db: Session = Depends(get_db)):
    ...
    unfiled_count = db.query(func.count(Item.id)).filter(Item.folder_id.is_(None)).scalar() or 0
...
def create_folder(request: FolderCreateRequest, db: Session = Depends(get_db)):
    ...
    if _find_folder_by_name(db, name):
        raise HTTPException(status_code=409, detail="Folder name already exists")

    now = datetime.datetime.utcnow()
    folder = Folder(name=name, created_at=now, updated_at=now)
    db.add(folder)
    db.commit()
    db.refresh(folder)
...
def update_folder(folder_id: str, request: FolderUpdateRequest, db: Session = Depends(get_db)):
    ...
    if not name:
        raise HTTPException(status_code=400, detail="Folder name is required")
    if _find_folder_by_name(db, name, exclude_folder_id=folder.id):
        raise HTTPException(status_code=409, detail="Folder name already exists")

    folder.name = name
    folder.updated_at = datetime.datetime.utcnow()
    db.commit()
    item_count = db.query(func.count(Item.id)).filter(Item.folder_id == folder.id).scalar() or 0
    db.refresh(folder)
    return FolderResponse(
        id=folder.id,
        name=folder.name,
...
def delete_folder(folder_id: str, db: Session = Depends(get_db)):
    ...
    db.query(Item).filter(Item.folder_id == folder.id).update({"folder_id": None})
    db.delete(folder)
    db.commit()
